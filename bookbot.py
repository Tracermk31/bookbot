...
def report(character_count, word_count, file_path):
    ...
    for character_dict in character_list:
        report_string += f"{character_dict['character']} appears {character_dict['count']} times in this book\n"
...
def main():
    file_path = input("Please enter the file path for the book you would like to read starting from the bookbot directory: ")
    book = read_book(file_path)
    word_count = count_words(book)
    character_count = count_character(book)
    book_report = report(character_count, word_count, file_path)
    print (book_report)
